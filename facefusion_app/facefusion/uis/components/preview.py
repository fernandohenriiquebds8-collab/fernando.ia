...
def update_preview_image(preview_mode : PreviewMode, preview_resolution : str, frame_number : int = 0) -> gradio.Image:
	...
	if is_video(state_manager.get_item('target_path')):
		...
		preview_vision_frame = cv2.cvtColor(preview_vision_frame, cv2.COLOR_BGRA2RGBA)
		return gradio.Image(value = preview_vision_frame, interactive = False, elem_classes = [ 'image-preview', 'is-' + detect_frame_orientation(preview_vision_frame) ])
	return gradio.Image(value = None, interactive = False, elem_classes = None)
