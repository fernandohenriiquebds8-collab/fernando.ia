...
def launch() -> None:
	...
	with gradio.Blocks(theme = get_theme(), css = get_css(), title = metadata.get('name') + ' ' + metadata.get('version'), fill_width = True, js = '() => { document.body.classList.add("dark"); }') as ui:
		for ui_layout in ui_layouts:
			ui_layout_module = load_ui_layout_module(ui_layout)
...
def get_theme() -> gradio.Theme:
	return gradio.themes.Base(
		...
		font = gradio.themes.GoogleFont('Outfit')
	).set(
		...
		block_border_color_dark = '#2c2c2f',
